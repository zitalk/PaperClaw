...
<!-- paperclaw-run: {"status": "degraded", "checked_at": "2026-09-11T01:29:48+08:00", "unavailable_sources": ["Semantic Scholar", "Springer Nature"], "unconfigured_sources": [], "filter_fallback": false, "failed_papers": 0} -->

## 📌 今日概况

检索完成，但存在异常 · 最近检查：2026-09-11 01:29:48（北京时间）

本轮检索候选论文 95 篇；刊会准入通过 1 篇（排除 94 篇）；本轮 LLM 新筛中 0 篇，复用已收录匹配 0 篇；本轮新增入报 0 篇；目标日累计收录 0 篇。当日未检索到符合条件并纳入日报的论文。 部分来源不可用：Semantic Scholar、Springer Nature；本次结果不代表完整覆盖。
