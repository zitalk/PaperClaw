...
| 缺失模态与鲁棒学习 | 开放词汇、零样本与免训练分割 | 时空一致性建模 | 低空场景与具身决策 |
...
论文检索由 GitHub Actions 远端在工作日执行，不依赖个人电脑开机。北京时间周一至周五 **03:00** 首次检查最近一个工作日的新论文，若 arXiv 尚未发布候选，则在 **09:30、12:30、15:30** 继续补查；周末不运行、不补扫，已完成日期会自动跳过。Pages 工作流会在日报更新后自动发布网站。
