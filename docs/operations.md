...
论文检索由 GitHub Actions 远端执行，不依赖个人电脑开机。北京时间周一至周五 **03:00、09:30、12:30、15:30、18:30、21:30、23:30** 分批检索，每轮只检索前一天，优先保证最新日报。周六、周日 **03:00** 各执行一次最近 **7 个自然日**的增量补漏；若发现遗漏，GitHub Actions 会生成 warning 和运行摘要，列出发现、补录与失败数量。论文仍按其真实日期写入对应日报。GitHub 调度可能排队，时间为计划触发时间，不保证准点完成。Pages 工作流会在日报更新后自动发布网站。
...
**统计分清本轮和累计。** 日报展示本轮候选、本轮 LLM 新筛中、复用匹配、本轮新增入报和目标日累计收录量，不把多轮候选数简单相加。零结果或部分来源不可用，不会让此前收录的论文从日报中消失。工作日多轮只追加前一天；迟到记录由周末七日窗口补回并明确提示，不宣称一次检索能穷尽所有来源。
